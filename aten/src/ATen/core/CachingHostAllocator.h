...
struct alignas(64) HostStatsStaged {
  ...
  // LOCK: access to this stat is protected by the per bucket free_list_[index].mutex_
  std::vector<Stat> allocation_bytes_bucket_stats = std::vector<Stat>(MAX_SIZE_INDEX);
 ...
 public:
  ...
  virtual void free(void* ctx) {
    ...
    if (!events) {
      ...
      stats_.allocation_bytes_bucket_stats[index].decrease(block->size_);
    } else {
  ...
  virtual void empty_cache() {
    ...
    for (size_t i = 0; i < free_list_.size(); ++i) {
      ...
      for (auto* block : blocks_to_remove) {
        ...
        stats_.allocation_bytes_bucket_stats[i].decrease(block->size_);
        free_block(block);
        delete block;
      }
    }
  ...
  HostStats getStats() {
    ...
    for (size_t i = 0; i < free_list_.size(); ++i) {
      ...
      // per bucket (we pick index 0 arbitrarily).
      if (i == 0) {
        stats.allocation = stats_.allocation;
        stats.allocated_bytes = stats_.allocated_bytes;
      }

      // Bucket stats need to be merged with the slow-path stats. We do this in
      // a best effort manner, since we can't really replay the cached events
      // per bucket.
      // @TODO: add tests
      // total allocation count adds bucket allocation counts
      // total allocated bytes adds bucket allocated bytes
      // active bytes adds only current bucket bytes
      // stats.allocation.increase(stats_.allocation_bucket_stats[index]);
      // stats.allocation_bytes.increase(stats_.allocation_bytes_bucket_stats[index]);
    }
  ...
  void resetAccumulatedStats() {
    ...
    for (size_t i = 0; i < free_list_.size(); ++i) {
      ...
      stats_.allocation_bytes_bucket_stats[i].reset_accumulated();
    }
  ...
  void resetPeakStats() {
    ...
    for (size_t i = 0; i < free_list_.size(); ++i) {
      ...
      stats_.allocation_bytes_bucket_stats[i].reset_peak();
    }
 ...
 private:
  ...
  virtual B* get_free_block(size_t size) {
    ...
    if (free_list_[index].list_.size() > 0) {
      ...
      stats_.allocation_bytes_bucket_stats[index].increase(size);
      return block;
    }
    return nullptr;
  }
  ...
  virtual void process_events_for_specific_size(int64_t size) {
    ...
    while (true) {
      ...
      if (available) {
        ...
        free_list_[index].list_.push_back(block);
        stats_.allocation_bucket_stats[index].decrease(1);
        stats_.allocation_bytes_bucket_stats[index].decrease(size);
        if (size != -1) {
          return;
        }
      }
